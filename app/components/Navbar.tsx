...
export default function Navbar() {
    ...
    return (
        <>
            <header
                className={`fixed z-[60] transition-all duration-500 ${isFloating
            ...
            >
                <div className="w-full max-w-7xl mx-auto px-6 md:px-8 h-20 flex items-center justify-between">
                    ...
                    {/* Logo Area */}
                    <Link href="/" className="relative flex items-center z-10" onClick={() => setIsMobileMenuOpen(false)}>
                    ...
                    {/* Desktop Navigation */}
                    <nav className={`hidden lg:flex gap-4 font-mono text-[14px] font-bold tracking-[0.2em] uppercase transition-colors duration-300 ${isScrolled ? "text-bio-primary" : "text-white"}`}>
                        <Link href="/" className="px-4 py-2 hover:text-bio-accent transition-colors">Home</Link>
                        <Link href="/technology" className={`px-4 py-2 hover:text-bio-accent transition-colors ${isScrolled ? "hover:text-bio-accent-dark" : "hover:text-bio-accent"}`}>Technology</Link>
                        <Link href="/company" className={`px-4 py-2 hover:text-bio-accent transition-colors ${isScrolled ? "hover:text-bio-accent-dark" : "hover:text-bio-accent"}`}>Company</Link>
                    </nav>

                    {/* Desktop Contact Detail */}
                    <div className="hidden lg:flex items-center">
                        <div className={`group flex items-center gap-3 font-mono text-[14px] font-bold tracking-[0.1em] transition-colors duration-300 ${!isScrolled ? "text-white" : "text-bio-primary"}`}>
                            <div className="relative flex items-center justify-center">
                                <motion.svg custom={0} variants={sparkleVariants} animate="animate" className="absolute -top-1 -left-1.5 w-2 h-2 text-bio-accent" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M12 0l3 9h9l-7 5 3 9-8-7-8 7 3-9-7-5h9z" />
                                </motion.svg>
                                <motion.svg custom={1} variants={sparkleVariants} animate="animate" className="absolute -bottom-1 -right-1.5 w-1.5 h-1.5 text-bio-accent" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M12 0l3 9h9l-7 5 3 9-8-7-8 7 3-9-7-5h9z" />
                                </motion.svg>
                                <motion.svg
                                ...
                                >
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                        <div className="flex flex-col gap-2">
                                            <span className="font-mono text-[12px] uppercase tracking-widest text-bio-primary/40 font-bold">Direct Inquiries</span>
                                            <a href="mailto:contact@biojena.com" className="font-mono text-lg text-bio-primary hover:text-bio-accent-dark transition-colors">
                                                contact@biojena.com
                                            </a>
                                            <span className="font-mono text-lg text-bio-primary">+49 3641 555 880</span>
                                        </div>
                                        <div className="flex flex-col gap-2">
                                            <span className="font-mono text-[12px] uppercase tracking-widest text-bio-primary/40 font-bold">Headquarters</span>
                                            <p className="font-mono text-[14px] text-bio-primary/60 leading-relaxed">
                                                BIOJENA GmbH<br />
                                                Spitzweidenweg 30<br />
                                                07743 Jena, Germany
                                            </p>
                                        </div>
                                    </div>
